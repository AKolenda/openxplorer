...
const VARIANTS=[
 ...
 ['A','Ledger — one column, numbered chapters'],
...
export function VariantA(){
 return <div className="site theme-zorin proto proto-a"><Header/><main id="main">
  <section className="shell a-hero"><p className="eyebrow">Made for your next chapter</p><h1>A familiar place.<br/>A fresh start.</h1><p className="lede">Windows File Explorer habits, native Linux storage. Type <code>\\nas\share</code> and you’re there.</p><ReleaseCta center/></section>
  ...
  <section className="shell a-chapter" id="features"><span className="a-num">01</span><div className="measure"><h2>Your NAS, addressed like you always have.</h2><p>The address bar accepts the paths you already know from Windows, plus the Linux ones. Names, IP addresses, forward or back slashes, smb:// URLs with a port. One sign-in per server, stored where you choose.</p></div><div className="a-addresses">{ADDRESSES.map(([v,c])=><AddressBar key={v} value={v} caption={c}/>)}</div><div className="a-points">{SMB_POINTS.map(([t,d])=><div key={t}><strong>{t}</strong><p>{d}</p></div>)}</div><Screenshot name="network-path" className="crop wide" alt="Actual OpenXplorer breadcrumb buttons for studio-nas, Projects and Design" caption="Each breadcrumb of a share is its own button."/></section>
  <section className="shell a-chapter"><span className="a-num">02</span><div className="measure"><h2>Drag it where you need it.</h2><p>Drag selected files into compatible editors and attachment fields. Drop onto a folder to copy, or into Quick access to pin it. Sources stay in place.</p><a className="text-link" href="/docs/interface/#file-drag-drop">File dragging &amp; compatibility <Icon name="arrow" size={16}/></a></div><Screenshot name="pinned-sidebar" className="crop tall" alt="Actual OpenXplorer sidebar with Design pinned and a green network indicator"/></section>
  <section className="shell a-chapter"><span className="a-num">03</span><div className="measure"><h2>Find the file. Keep the path.</h2><p>Opt selected folders into a local filename cache. Search locally or on a share, then open the actual location instead of another search maze.</p><a className="text-link" href="/docs/search-indexing/">How cached search works <Icon name="arrow" size={16}/></a></div><Screenshot name="cached-search" className="crop" alt="Real cached search results with filenames and full SMB folder paths"/></section>
  <section className="shell a-chapter"><span className="a-num">04</span><div className="measure"><h2>The right moment, clearly marked.</h2><p>Browse exposed NAS snapshots with the date beside each version. Historical tabs carry a Previous version badge so you always know where you are.</p><a className="text-link" href="/docs/interface/#snapshots">Explore previous versions <Icon name="arrow" size={16}/></a></div><Screenshot name="previous-versions" className="crop mid" alt="Previous versions of the fictional Launch planning folder, with compact dates and separate actions"/></section>
  <section id="demo" className="shell a-chapter desktop-demo-only"><span className="a-num">05</span><div className="measure"><h2>Take the familiar route.</h2><p>Type a path. Open the NAS. Pin a folder. Try the interface before installing anything.</p></div><ProductDemo/></section>
  <section className="shell a-end"><div className="measure"><span className="eyebrow">Open by choice</span><h2>Yours to use. Yours to understand.</h2><p>The desktop app, this website, the docs and the build tools are all public under AGPL-3.0-only. Nothing behind a login.</p><ReleaseCta/></div></section>
