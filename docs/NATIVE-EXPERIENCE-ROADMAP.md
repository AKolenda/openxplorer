...
| P3 | Undo, cross-filesystem moves and conflict merging | These need a separate data-safety design. The engine explicitly disallows overwrite and uses native-only moves; there is no undo journal. Durable recovery, metadata policy, conflict decisions, partial failure and restart behavior must precede UI promises. Do not implement a drag move as unverified copy-then-delete. |
